...
class OnPolicyRewardAlgorithm_Explore(BaseAlgorithm):
    """
    ...
    def __init__(
        ...
        super(OnPolicyRewardAlgorithm_Explore, self).__init__(
            policy=policy,
            env=env,
    ...
    def collect_rollouts(
        self, env: VecEnv, callback: BaseCallback, rollout_buffer: RolloutBuffer, n_rollout_steps: int, train_steps: int
    ) -> bool:
        """
        ...
        if self.beta_schedule == "constant":
            ...
            beta = self.beta_init * ((1 - self.beta_decay) ** train_steps)
        else:
        ...
        while n_steps < n_rollout_steps:
            ...
            reward_hat, reward_std = self.reward_model.r_hat_std_batch(obsact)
            pred_reward = reward_hat.reshape(-1) + reward_std.reshape(-1) * beta
    ...
    def learn(
        ...
        tb_log_name: str = "OnPolicyRewardAlgorithm_Explore",
        eval_log_path: Optional[str] = None,
        reset_num_timesteps: bool = True,
    ) -> "OnPolicyRewardAlgorithm_Explore":
        iteration = 0
        ...
        while self.num_timesteps < total_timesteps:
            continue_training = self.collect_rollouts(self.env, callback, self.rollout_buffer, n_rollout_steps=self.n_steps, train_steps=self.num_timesteps)
